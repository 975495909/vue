            ...
            <el-container>
                        ...
                        <el-dialog v-model="dialogFormVisible" :title="diaTitle" width="60%" @close="formClose">
                            <el-form :model="form" :rules="rules" label-width="30%" ref="form">
                                ...
                                <el-form-item label="车牌号" prop="carNum">
                                    <el-input v-model="form.carNum" clearable />
                                </el-form-item>
                        ...
                        <el-input class="inpSearch" v-model="searchMap.selcontent" placeholder="请输入"
                            ...
                            <template #append>
                                <el-select v-model="searchMap.sel" style="width: 150px;background-color: #f3efff;">
                                    ...
                                    <el-option label="车牌号" value="carNum" />
                                    <el-option label="车辆颜色" value="color" />
                                </el-select>
                            </template>
                        </el-input>
                        <!-- 日期选择器 -->
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="carNum" label="车牌号" width="300" />
                        <el-table-column prop="color" label="车辆颜色" width="300" />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //分页+搜索查看
            search() {
                axios.post("/car/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/car/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
            ...
            formClose() {
                ...
                this.imagesUrl.forEach(item => {
                    axios.get("/file/delcar?fileName=" + item).then((res) => {
                        if (res.flag) {
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        axios.post("/car/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    dataDel.forEach(item => {
                        ...
                        axios.get("/file/delcar?fileName=" + pic).then((res) => {
                            if (res.flag) {
                                console.log("文件：【" + pic + "】删除成功")
                            }
                        });
                    })
                    axios.post("/car/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
