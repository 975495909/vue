            ...
            <el-container>
                        ...
                        <el-input class="inpSearch" v-model="searchMap.selcontent" placeholder="请输入"
                            ...
                            <template #append>
                                <el-select v-model="searchMap.sel" style="width: 150px;background-color: #f3efff;">
                                    ...
                                    <el-option label="宠物品种" value="name" />
                                    <el-option label="宠物颜色" value="color" />
                                </el-select>
                            </template>
                        </el-input>
                        <!-- 日期选择器 -->
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="name" label="宠物品种" width="300" />
                        <el-table-column prop="color" label="宠物颜色" width="300" />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //分页+搜索查看
            search() {
                axios.post("/pet/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/pet/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
            ...
            formClose() {
                ...
                this.imagesUrl.forEach(item => {
                    axios.get("/file/delpet?fileName=" + item).then((res) => {
                        if (res.flag) {
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        axios.post("/pet/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    dataDel.forEach(item => {
                        ...
                        axios.get("/file/delpet?fileName=" + pic).then((res) => {
                            if (res.flag) {
            ...
            handleDelete() {
                ...
                this.delIds.push(id)
                axios.post("/pet/del", this.delIds).then((res) => {
                    if (res.flag) {
                        this.$message({
                            message: res.message,
                            type: 'success'
