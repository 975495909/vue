...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/repair/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/repair/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
                        //选择器
                        this.searchOptions2()
                    });
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/repair/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/repair/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
