            ...
            <el-container>
                        ...
                        <el-dialog v-model="dialogFormVisible" :title="diaTitle" width="60%">
                            <el-form :model="form" :rules="rules" label-width="30%" ref="form">
                                ...
                                <el-form-item label="楼层" prop="floor">
                                    <el-input v-model="form.floor" clearable />
                                </el-form-item>
                                <el-form-item label="房间号" prop="houseNum">
                                    <el-input v-model="form.houseNum" clearable />
                                </el-form-item>
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="floor" label="楼层" width="130" />
                        ...
                        <el-table-column fixed="right" prop="status" label="状态" width="130" sortable><template
                                #default="scope">
                                <el-tag :type="scope.row.status== '0' ? '': scope.row.status== '1'?'warning':'success'">
                                    {{scope.row.status== '0' ? '未入住' : scope.row.status== '1'?'已出租':'已被购买'}}
                                </el-tag>
                            </template>
                        </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/house/selpage", this.searchMap).then((res) => {
                    ...
                    if (res == null || res.length == 0) this.loading = true
                        ...
                        res.data.forEach(item => {
                            ...
                            axios.get("/property/selbyhouseid?id=" + Id).then((
                                res) => {
                                //已被购买2
                                if (res.data.length != 0 && item.ownerId == 0) {
                                    item.status = 2
                                    item.ownerId = res.data[0].ownerId
                                    item.ownerName = res.data[0].ownerName
                                    axios.post("/house/update", item).then((res) => {
                                    })
                                } else if(res.data.length == 0){
                                    axios.get("/lease/selbyhouseid?id=" + Id).then((
                                        res) => {
                                        //已出租1
                                        if (res.data.length != 0 && item.ownerId == 0) {
                                            item.status = 1
                                            item.ownerId = res.data[0].ownerId
                                            item.ownerName = res.data[0].ownerName
                                            axios.post("/house/update", item)
                                                .then((
                                                    res) => {
                                                })
                                        }
                                        // 未入住0
                                        else if (res.data.length == 0 && item.ownerId !=
                                            0) {
                                            item.status = 0
                                            item.ownerId = 0
                                            item.ownerName = '无'
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/house/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
                        //选择器
                        // this.searchOptions2()
                    });
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/house/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/house/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
            ...
            //         axios.get("/owner/selbybuildingid?id=" + this.form.buildingId).then((res) => {
            //             if (res == null || res.length == 0) this.loading = true
            //             else {
            //                 this.loading = false
