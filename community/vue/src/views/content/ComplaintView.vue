...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/complaint/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/complaint/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
                        //选择器
                        this.searchOptions2()
                    });
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/complaint/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/complaint/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
