            ...
            <el-container>
                        ...
                        <el-dialog v-model="dialogFormVisible" :title="diaTitle" width="60%">
                            <el-form :model="form" :rules="rules" label-width="30%" ref="form">
                                ...
                                <el-form-item label="总户数" prop="totalHouseholds">
                                    <el-input v-model="form.totalHouseholds" clearable />
                                </el-form-item>
                        ...
                        <el-input class="inpSearch" v-model="searchMap.selcontent" placeholder="请输入"
                            ...
                            <template #append>
                                <el-select v-model="searchMap.sel" style="width: 150px;background-color: #f3efff;">
                                    <el-option label="楼栋名称" value="name" />
                                </el-select>
                            </template>
                        </el-input>
                        <!-- 日期选择器 -->
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="name" label="楼栋名称" width="300" />
                        <el-table-column prop="totalHouseholds" label="总户数" width="150" />
...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/building/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/building/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
                    });
                }
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/building/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/building/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
