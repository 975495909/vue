            ...
            <el-container>
                        ...
                        <el-input class="inpSearch" v-model="searchMap.selcontent" placeholder="请输入"
                            ...
                            <template #append>
                                <el-select v-model="searchMap.sel" style="width: 150px;background-color: #f3efff;">
                                    <el-option label="活动主题" value="title" />
                                    <el-option label="举办单位" value="organizer" />
                                </el-select>
                            </template>
                        </el-input>
                        <!-- 日期选择器 -->
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="address" label="活动地点" width="300" />
                        <el-table-column prop="organizer" label="举办单位" width="300" />
                        <el-table-column prop="startingTime" label="活动开始时间" width="180" />
                        <el-table-column prop="endingTime" label="活动结束时间" width="180" />
                        <el-table-column prop="description" label="描述" width="526" />
                        ...
                        <el-table-column fixed="right" prop="status" label="状态" width="130" sortable><template
                                #default="scope">
                                <el-tag :type="scope.row.status== '0' ? '': scope.row.status== '1'?'danger':'success'">
                                    {{scope.row.status== '0' ? '未开始' : scope.row.status== '1'?'已结束':'正在进行中'}}
                                </el-tag>
                            </template>
                        </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/activity/selpage", this.searchMap).then((res) => {
                    ...
                    if (res == null || res.length == 0) this.loading = true
                        ...
                        res.data.forEach(item => {
                            ...
                            const end = new Date(JSON.parse(JSON.stringify(item)).endingTime)
                            //获取现在日期
                            const now = new Date()
                            //未开始0
                            if (now < start) {
                                if (item.status != 0) {
                                    item.status = 0
                                    // console.log(item)
                                    axios.post("/activity/update", item).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/activity/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/activity/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/activity/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
