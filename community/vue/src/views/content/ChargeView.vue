            ...
            <el-container>
                    ...
                    <div class="head">
                        ...
                        <el-dialog v-model="dialogFormVisible" :title="diaTitle" width="60%">
                            <el-form :model="form" :rules="rules" label-width="30%" ref="form">
                                ...
                                <el-form-item label="类型名称" prop="chargeName">
                                    <el-select v-model="form.chargeId" clearable filterable placeholder=" "
                                        @change="selected()">
                                        <el-option v-for="item in options" :key="item.id" :label="item.name"
                                            :value="item.id" />
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="承办人姓名" prop="contrator">
                                    <el-input v-model="form.contrator" clearable />
                                </el-form-item>
                                <el-form-item label="手机号" prop="telephone">
                                    <el-input v-model="form.telephone" clearable />
                                </el-form-item>
                                <el-form-item label="应收金额(¥)" prop="payMoney">
                                    <el-input v-model="form.payMoney" clearable />
                                </el-form-item>
                                <el-form-item label="实收金额(¥)" prop="actualMoney">
                                    <el-input v-model="form.actualMoney" clearable />
                                </el-form-item>
                                <el-form-item label="缴费日期" prop="purchaseDate">
                        ...
                        <el-input class="inpSearch" v-model="searchMap.selcontent" placeholder="请输入"
                            ...
                            <template #append>
                                <el-select v-model="searchMap.sel" style="width: 150px;background-color: #f3efff;">
                                    ...
                                    <el-option label="类型名称" value="chargeName" />
                                    <el-option label="承办人姓名" value="contrator" />
                ...
                <el-main>
                    ...
                    <el-table v-loading="loading" element-loading-text="拼命加载中..." :data="List" border size="large"
                        ...
                        <el-table-column prop="chargeName" label="类型名称" width="300" />
                        <el-table-column prop="contrator" label="承办人姓名" width="130" />
                        <el-table-column prop="telephone" label="手机号" width="300" />
                        <el-table-column prop="payMoney" label="应收金额(¥)" width="150" />
                        <el-table-column prop="actualMoney" label="实收金额(¥)" width="150" />
                        <el-table-column prop="purchaseDate" label="缴费日期" width="180" />
...
<script>
    ...
    export default {
        ...
        methods: {
            //分页+搜索查看
            search() {
                axios.post("/charge/selpage", this.searchMap).then((res) => {
            ...
            handleDialog(val, row) {
                if (val == 1) {
                    ...
                    axios.get("/charge/selbyid?id=" + id).then((res) => {
                        this.form = res.data;
                    });
                }
            ...
            handleSave() {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        // console.log(obj)
                        axios.post("/charge/" + method, obj).then((res) => {
                            if (res.flag) {
                                this.$message({
                                    message: res.message,
                                    type: 'success'
            ...
            handleDel() {
                ...
                if (dataDel.length != 0) {
                    ...
                    axios.post("/charge/del", this.delIds).then((res) => {
                        if (res.flag) {
                            this.$message({
                                message: res.message,
                                type: 'success'
            ...
            //选择器的数据
            searchOptions() {
                axios.get("/chargetype/selall").then((res) => {
                    // console.log(this.searchMap)
                    if (res == null || res.length == 0) this.loading = true
                    else {
                        this.loading = false
            ...
            selected() {
                axios.get("/chargetype/selbyid?id=" + this.form.chargeId).then((res) => {
                    this.form.chargeName = res.data.name
                    // console.log(res)
                });
            },
            //日期格式化方法——直接获得
